...
def run_cmd(args, ok_fail=False, capture=False, stdin_data=None):
    ...
    return None, result.resultcode

def kubectl_apply_yaml(yaml_str):
    result = subprocess.run(
        ["kubectl", "apply", "-f", "-"], input=yaml_str, text=True
    )
...
print("  creates the pharma AppProject and deploy Applications")
print()
...
GITOPS_REPO_URL = prompt("GITOPS_REPO_URL", "GitOps repository HTTPS URL", 
                         ...
                         "https://github.com/zenpharma-ccs/gitops.git")
...
GITHUB_USERNAME = prompt("GITHUB_USERNAME", "Your personal GitHub username",
                         "<your-github-username>", "johanccs")
GITHUB_TOKEN = prompt_secret("GITOPS_TOKEN", 
...
# Step 1 of 3 - Register GitOps repository
...
dry_run_result = subprocess.run(
    [
        ...
        f"--from-literal=password={GITHUB_TOKEN}",
        "--dry-run=client", "-o", "yaml"
    ],
    capture_output=True, text=True
)

if dry_run_result.resultcode != 0:
